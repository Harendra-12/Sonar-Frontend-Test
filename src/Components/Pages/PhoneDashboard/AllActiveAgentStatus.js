    ...
    setAllParkedCall(
      activeCall.filter(
        (call) =>
          ...
          (call.b_callee_direction !== "ACTIVE" ||
            call.b_callee_direction !== "HELD")
      )
    );
  }, [activeCall]);
  ...
  return (
    <>
      ...
      >
        {!isActiveCallsPage && (
          <button
            onClick={() => setIsActiveAgentsOpen(!isActiveAgentsOpen)}
            ...
            <div className="d-flex flex-wrap">
              <div className="col-12">
                <div
                  className="heading"
                  style={
                    isActiveCallsPage ? { paddingLeft: "55px" } : undefined
                  }
                >
                  <div className="content mb-0">
                    <h4>
                      {isParentWebRtc ? "Additional Info" : "Agent Status"}
                    </h4>
                    <p>{`You can see all of the ${
                      isParentWebRtc
                        ? "parked calls & agent activity status"
                    ...
                    <div className="col-12">
                      <nav className="tangoNavs">
                        <div
                          ...
                          role="tablist"
                          style={
                            !isParentWebRtc ? { height: "65px" } : undefined
                          }
                        >
                          <button
                            className="nav-link active"
                              ...
                              <tbody>
                                {allUser?.length > 0 &&
                                  allUser
                                    ...
                                    .map((agent, index) => {
                                      const activeCallsForAgent =
                                        activeCall.filter(
                                          (call) =>
                                            call?.dest ===
                                              agent?.extension?.extension ||
                                            call?.b_presence_id?.split(
                                              "@"
                                            )[0] ===
                                              agent?.extension?.extension ||
                                            call?.cid_num ===
                                      ...
                                      const getCallStatus = () => {
                                        if (activeCallsForAgent.length === 0)
                                          return null;

                                        const activeCall =
                                          activeCallsForAgent.filter(
                                            (call) =>
                                              call?.b_callstate === "ACTIVE" ||
                                              call?.b_callstate === "HELD" ||
                                        ...
                                        if (
                                          activeCall[0]?.b_callstate ===
                                            "ACTIVE" ||
                                          activeCall[0]?.b_callstate ===
                                            "HELD" ||
                                          activeCall[0]?.callstate === "HELD"
                                        ) {
                                          return {
                                            status:
                                              activeCall[0]?.callstate ===
                                                "HELD" ||
                                              activeCall[0]?.b_callstate ===
                                                "HELD"
                                                ? "On Hold"
                                ...
                                allParkedCall.map((call, index) => {
                                  return (
                                    <tr key={index}>
                                      ...
                                      <td>
                                        {extractLastNumber(call?.parked_by)}
                                      </td>
                                      <td>
