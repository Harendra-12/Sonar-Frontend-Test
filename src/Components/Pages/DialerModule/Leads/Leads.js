  ...
  const getLead = async () => {
    ...
    try {
      const res = await generalGetFunction(
        `/lead-file/all?page=${pageNumber}&row_per_page=${itemsPerPage}&search=${searchQuery}`
      );
      if (res?.status) {
        dispatch({
    ...
    <main className="mainContent">
      ...
      {leadEditPopup && (
        <LeadEditPopup
          ...
          setLeadEditPopup={setLeadEditPopup}
        />
      )}
    ...
    <>
      ...
      <div className="addNewContactPopup">
        <button
          ...
          onClick={() => setLeadEditPopup(false)}
        >
