...
const CDRTracker = () => {
  ...
  const getAllCDRReports = async (shouldLoad) => {
    ...
    const response = await generalGetFunction(
      api_url?.FPORTAL_ALL_CDR(pageNumber, itemsPerPage, searchValue)
    );
    if (response.status) {
                          ...
                          {loading ? (
                            ...
                            allCdrReport?.data?.map((item, index) => (
                              <tr key={index}>
                                ...
                                <td>{item?.dest}</td>
                                <td>
                                  {formatTimeSecondsToHHMMSS(item?.duration)}
                                </td>
