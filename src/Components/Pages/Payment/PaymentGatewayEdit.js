...
import { generalPostFunction, generalPutFunction } from "../../GlobalFunction/globalFunction";
...
function PaymentGatewayEdit() {
  ...
  useEffect(() => {
    if (locationState === null || locationState === undefined) {
      ...
      setPass(locationState.password);
      setKey(locationState.api_key);
      setSecret(locationState.api_secret);
  ...
  async function handleSubmit() {
    if (name === "") {
      ...
      const apiData = await generalPutFunction(
        `/payment-gateway/update/${locationState.id}`,
        parsedData
      );
      if (apiData.status) {
        setLoading(false);
