...
export const getAllMessageApiFun = async (pageNumb, recipient, messageListRef, scrollPositionRef, allAgents, setAllMessage, chatHistory, setChatHistory, setPageLoader) => {
    ...
    apiData?.data?.data?.map((item) => {
        ...
        const formattedMessages = apiData?.data?.data.map((item) => {
            const user_details = allAgents?.find(agent => agent?.id == item?.user_id);
            return {
                ...item,
                from: item.user_id,
        ...
        const updatedAllMessage = allMessage[conversationKey]?.map((msg) => {
            if (msg.id === result?.data?.id) {
                return { 
                    ...
                    edit_status: result?.data?.edit_status
                 };
            } else {
                return { ...msg }
            }
...
export const receiveGroupMessage = (allNotificationState, groupMessage, setAllMessage, groups, setGroups, setOriginalGroupsList, setActiveTab, setUnreadMessage, recipient, ActionType, dispatch, account) => {
